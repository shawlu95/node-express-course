### Note
Disadvantage of one-token cookie: user will be logged out when cookie expires. Better to have two tokens:
* `accessToken` very short expiration
* `refreshToken` long lived. When refreshed, renew the access token
* Proxy is misleading. Use `req.get('x-forwarded-host');` to dynamically get the origin.
* Hash token deterministically to make password recovery safer.
* Front end app's `proxy` must match server app's URI and port.
#### Front End App
